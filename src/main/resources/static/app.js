...
const STATUS_TRANSITIONS = {
    ...
    APPLIED: ["APPLIED", "ONLINE_ASSESSMENT", "VIDEO_INTERVIEW", "REJECTED", "WITHDRAWN"],
    ONLINE_ASSESSMENT: ["ONLINE_ASSESSMENT", "VIDEO_INTERVIEW", "ASSESSMENT_CENTRE", "REJECTED", "WITHDRAWN"],
    VIDEO_INTERVIEW: ["VIDEO_INTERVIEW", "ASSESSMENT_CENTRE", "FINAL_INTERVIEW", "REJECTED", "WITHDRAWN"],
    ASSESSMENT_CENTRE: ["ASSESSMENT_CENTRE", "FINAL_INTERVIEW", "OFFER", "REJECTED", "WITHDRAWN"],
    FINAL_INTERVIEW: ["FINAL_INTERVIEW", "OFFER", "REJECTED", "WITHDRAWN"],
...
document.addEventListener("DOMContentLoaded", async () => {
    populateStatusOptions(ALL_STATUSES, "SAVED");
    setDefaultApplicationDate();

    await loadSummaryApplications();
    await loadFilteredApplications();
});
...
statusFilter.addEventListener("change", loadFilteredApplications);
priorityFilter.addEventListener("change", loadFilteredApplications);
sortSelect.addEventListener("change", loadFilteredApplications);
...
async function loadSummaryApplications() {
    try {
        const response = await fetch(API_URL);

        if (!response.ok) {
            throw new Error("Unable to load application summary");
        }
...
async function loadFilteredApplications() {
    try {
        ...
        if (sortSelect.value === "PRIORITY") {
            filteredApplications.sort(comparePriority);
        }
...
function mapFrontendSortToBackendSort(sortValue) {
    switch (sortValue) {
        case "DEADLINE_ASC":
            return "deadline";
            ...
            .map(application => {
                ...
                return `
                    <tr>
                        ...
                        <td>
                            <span class="badge ${getPriorityClass(application.priority)}">
                        ...
                        <td>
                            <span class="badge ${deadlineHealth.className}">
...
function getDeadlineHealth(application) {
...
function editApplicationFromButton(button) {
    const encodedApplication =
